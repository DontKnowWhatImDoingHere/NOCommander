...
internal sealed partial class CommanderSupplyHeliService
{
    ...
    internal void ResetSession()
    {
        ...
        CommanderSupplyDebugTelemetry.Reset();
    }
    ...
    internal void RefreshOptions()
    {
        ...
        RefreshAirbaseOptions();
        CommanderPlugin.Log.LogInfo($"Commander supply aircraft refreshed: aircraft={aircraftOptions.Count}, cargoSlots={CountCargoSlots()}");
        for (int i = 0; i < aircraftOptions.Count; i++)
        {
            CargoAircraftOption option = aircraftOptions[i];
            CommanderPlugin.Log.LogInfo($"Supply aircraft option[{i}]: {option.Label}; slots={string.Join(", ", option.CargoSlots.ConvertAll(slot => $"{slot.Label}({slot.Mounts.Count})"))}");
        }
    }
