...
internal sealed partial class CommanderSupplyHeliService
{
        ...
        IReadOnlyList<GlobalPosition> targets)
    {
        ...
        if (spawnAirbase == null || pendingAircraftSpawn != null)
        {
            ...
            CommanderPlugin.Log.LogInfo($"Supply cargo run queued: aircraft={aircraftOption.Label}, requestedAirbase={GetAirbaseLabel(airbase)}, useOtherAirfields={useOtherAirfields}, queue={queuedCargoSpawns.Count}");
            return;
        }
    ...
    private void TrySpawnCargoRunAtAirbase(QueuedCargoSpawn request, Airbase airbase, FactionHQ hq)
    {
        ...
        CommanderPlugin.Log.LogInfo($"Supply cargo run spawned: aircraft={aircraftOption.Label}, cargo={request.CargoLabel}, airbase={GetAirbaseLabel(airbase)}, target={request.Target}, highClearance={request.HighTerrainClearance}, airdrop={request.Airdrop}, support={request.SupportSummary}, purchased={purchased}");
    }
    ...
    private void TryAssignPendingAircraft(FactionHQ hq, Unit unit)
    {
        ...
        CommanderSupplyDebugTelemetry.TrackAircraft(aircraft, pending.Target, pending.HighTerrainClearance, pending.Airdrop);
        pendingAircraftSpawn = null;
        CommanderPlugin.Log.LogInfo($"Supply cargo target assigned: aircraft={CommanderGameAccess.GetUnitLabel(aircraft)}, cargo={pending.CargoLabel}, target={pending.Target}");
    }
    ...
    private void EndTargetOverrideForState(AIHeloTransportState state)
    {
            ...
            && mission.TargetOverrideActive)
        {
            ...
            CommanderPlugin.Log.LogInfo($"Supply cargo mission completed or left transport state: {CommanderGameAccess.GetUnitLabel(aircraft)}");
        }
    }
    ...
    private void HoldDeployedCargo(Aircraft aircraft, Unit cargoUnit)
    {
        ...
        CommanderPlugin.Log.LogInfo(
            $"Supply cargo activated: aircraft={CommanderGameAccess.GetUnitLabel(aircraft)}, cargo={CommanderGameAccess.GetUnitLabel(cargoUnit)}, remaining={CountDeployableCargo(aircraft)}");
    }
    ...
    private bool DeployNextAssignedCargo(AIHeloTransportState state)
    {
        ...
        int before = CountDeployableCargo(aircraft);
        ...
        int after = CountDeployableCargo(aircraft);
        ...
        CommanderPlugin.Log.LogInfo(
            $"Supply cargo release: aircraft={CommanderGameAccess.GetUnitLabel(aircraft)}, station={station.Number}, before={before}, remaining={after}");
        return true;
    }
    ...
    private static IEnumerator ClearStaticCargoFromRamp(Aircraft aircraft, Unit cargo, CargoMission mission)
    {
        ...
        {
            CommanderPlugin.Log.LogInfo(
                $"Supply cargo cleared from ramp: {CommanderGameAccess.GetUnitLabel(cargo)}");
        }
    }
    ...
    private void HandleAircraftReturned(Aircraft aircraft)
    {
        ...
        CommanderPlugin.Log.LogInfo(
            $"Supply aircraft purchase refunded after return: aircraft={CommanderGameAccess.GetUnitLabel(aircraft)}, funds={mission.PurchaseCost:F1}");
    }
    ...
    private void PruneFinishedMissions()
    {
        ...
        for (int i = 0; i < stale.Count; i++)
        {
            ...
            CommanderSupplyDebugTelemetry.NotifyAircraftEnded(aircraft);
        }
    }
