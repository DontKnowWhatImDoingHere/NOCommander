    ...
    private void DrawWindow(int windowId)
    {
        if (CommanderUiTheme.DrawHelpButton(windowRect.width, ref helpVisible))
        {
            CommanderUiTheme.DrawHelpOverlay(
                ...
                "Click vehicles to stage them. SPAWN transfers the staged list to the expandable depot queue. Reserve Only shows vehicles already held by the faction. Rally orders are applied after the Basegame depot exit order.");
        }
